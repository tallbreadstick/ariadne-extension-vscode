...
### Common Vulnerabilities — Hybrid Counting and Post-Fix Graduation Probation

- date: 2026-09-19
- status: accepted
- context: An alignment review and user testing identified two requirements in the Common Vulnerabilities engine. (1) Session counting for entry threshold K=3 maps to hourly checkpoints in a 3-hour lab session, counting presence across milestones, while total instance count represents distinct FLCs. (2) Graduation requires both fixing and prevention. Previously, graduation only checked whether new instances were created (`!csNewTypes[i]`), ignoring whether persisting instances were actively unresolved during those completed sessions. This allowed a student to leave a vulnerability unresolved across G=2 sessions, fix it in the active session, and have it graduate immediately without any post-fix probation.
- decision: Implement hybrid counting and true post-fix probation in `commonVulnerabilities.ts`.
  1. **Counting**: Milestone presence increments `sessionCount` toward K=3. Findings resolved or missing prior to session start are excluded from completed and active session milestone presence, preventing historical resolved findings from inflating session counts in future sessions. Total FLC count is tracked independently as `totalInstanceCount` and serves as primary card sort key.
  2. **Graduation**: Graduation requires `allResolved` on `currentLifecycles` AND a post-fix probation period of `G=2` consecutive clean completed sessions completed *after* the session where the vulnerability was first introduced (`i > firstSeenCompletedSessionIdx`). A completed session is clean for a type if and only if: (a) no new instances of the type were created, (b) no active instances existed at session end (`durableResolutionAt === null && missingSince === null` in `lifecycleSummaries`), and (c) no active instances were detected at hourly checkpoints during the session (`hourlyCheckpoints`).
  3. **Resurrection Prevention**: A graduated vulnerability type with 0 active findings is permanently graduated and never resurrected as "All Resolved". Only genuine reintroductions with active findings (`activeFindingCount > 0`) can accumulate towards re-entry. In addition, graduation history is persisted on every computation pass in `buildCurrentSessionMetrics()`.
- consequences: Inaction on active vulnerabilities does not satisfy the graduation wait time. Brand-new vulnerabilities resolved in their first session remain on the panel as "All Resolved" until 2 completed clean sessions pass. Graduated vulnerabilities stay graduated and never randomly reappear. No storage migration required.
- supersedes: Common Vulnerabilities — Cross-Session Type-Level Awareness Metric
